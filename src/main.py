...
def cadastro_user():
    ...
    endereco = tela_usuario.lineEdit_5.text()
    ...
    if (senha == senhacf):
        try:
            with conexao.cursor() as cursor:
                ...
                cursor.execute(sql, (email, senha, nascimento, endereco, nome, CPF, RG, CNH))
                conexao.commit()
...
def editar_user():
    ...
    with conexao.cursor() as cursor:
        ...
        tela_consulta.lineEdit_8.setText(str(resultado[linha][8]))
...
tela_usuario.lineEdit_4.setInputMask("99/99/9999")
tela_consulta.lineEdit_3.setInputMask("99/99/9999")
